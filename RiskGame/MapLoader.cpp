		...
		while (getline(input, line)) {
			if (line == CONTINENTS_HEADER) {
				...
				if (position_counter < 2) {
					throw exception(
						("Invalid '[Continents]' section. Line \"" + line +
							"\" is not in the correct format of: 'Continent'='Bonus'").c_str());
				}
			}
			...
			if (line == TERRITORIES_HEADER) {
				...
				if (position_counter < 5) {
					throw exception(("Invalid '[Territories]' section. Line \"" + line +
						"\" is not in the correct format of: 'Territory','x-coord','y-coord','Continent','Adjacencies',..."
					).c_str());
				}
			}
		}
	} else {
		throw exception("Input file cannot be read (invalid text file format/file cannot be found)");
	}
